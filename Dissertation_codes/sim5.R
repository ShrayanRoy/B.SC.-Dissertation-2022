...
# Here We will do a comparative study of Wilcoxon Rank Sum test &
...
#Equal Variance
Power_comparison.T3 <- function(n1,n2,d,mu,R,alpha,exact.crit = F){
  ...
  colnames(power.matrix) = c('Wilcoxon Ranksum test','T Test')
  
  for(i in 1:length(d)){
    
    test.statistic <- replicate(R,{
      
      x <- rnorm(n1,mu + d[i],1); y <- rnorm(n2,mu,1) #our sample
      ...
      #Storing the values 
      c(WilcoxonRankSum.statistic,T.statistic)
    })
    
    #Simulated power for Wilcoxon RankSum test
    if(exact.crit){
    ...
    power.matrix[i,] <- c(WilcoxonRankSum.power,T.power)
  }
...
#Function to draw the power curve
Visualize_Power_comparison.T3 <- function(n1,n2,d,mu,R,alpha,exact.crit =
                                            F){
  
  #Storing Simulated Powers
  M <- Power_comparison.T3(n1,n2,d,mu,R,alpha,exact.crit)
  ...
  +
    ...
    ggtitle('Simulated Power Curve of Wilcoxon Ranksum Test & T Test') +
    theme_bw(14)
