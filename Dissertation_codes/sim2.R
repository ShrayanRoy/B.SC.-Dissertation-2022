...
Distribution
Power_comparison.LRT1 <- function(n1,n2,d,lamda,R,alpha,exact.crit = F){
  ...
  colnames(power.matrix) = c('Kolmogorov Smirnov Test','Exponential LRT')
  ...
  for(i in 1:length(d)){
    ...
    test.statistic <- replicate(R,{
      ...
      #Storing the values 
      c(Kolmogorov_smirnov.statistic,Exponential_LRT.statistic)
    })
    ...
    power.matrix[i,] <- c(Kolmogorov_smirnov.power,Exponential_LRT.power)
  }
...
Visualize_Power_comparison.LRT1 <-
  function(n1,n2,d,lamda,R,alpha,exact.crit = F){
    
    #Storing Simulated Powers
    M <- Power_comparison.LRT1(n1,n2,d,lamda,R,alpha,exact.crit)
    ...
    + geom_point(show.legend = F,size = 2) +
      ...
      ggtitle('Simulated Power Curve of Kolmogorov Smirnov Test & 
Exponential LRT') +
      theme_bw(14)
