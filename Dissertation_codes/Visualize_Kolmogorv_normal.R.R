...
visualize3 <- function(n1,n2,d,R,alpha,crit.value){
  ...
  index.1 <- factor(rep(c('Kolmogorov Smirnov Statistic','T statistic'),each = nrow(graph.matrix)),levels = c('Kolmogorov Smirnov Statistic','T statistic'))
  ...
  my.intercept <- function(x){
    ifelse(x == 'Kolmogorov Smirnov Statistic',crit.value,qt(alpha,n1+n2-2,lower.tail = F))
  }
  ...
  graph.1 <- ggplot(graph.data,aes(value,fill = Index)) +
    ...
    ggtitle('Kolmogorov Smirnov statistic & T statistic')+ facet_wrap(.~ Index,scales = 'free_x') +
  ...
  graph.2 <- graph.1 + labs(subtitle = 'Difference : {frame_time}',x = ' ',y = 'Density',caption = paste('Note: n1 =',n1,',n2 = ',n2,'& R = ',R)) +
    ...
    ggtitle('  Simulated Distribution : \n Kolmogorov Smirnov statistic & T statistic') + transition_time(d) + ease_aes('linear')
...
visualize3(20,16,seq(0,4,by = 0.2),1000,0.05,17/30)
