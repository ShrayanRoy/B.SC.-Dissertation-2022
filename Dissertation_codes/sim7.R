...
# Here We will do a comparative study of Different Non-Parametric test &
# T test in two sample problems, where data is from Normal Distribution
#Equal Variance
Power_comparisonALL.1 <- function(n1,n2,d,mu,R,alpha,exact.crit =c(F,F,F)){
  ...
  colnames(power.matrix) = c('MNW Test','Wilcoxon Rank-Sum Test','Kolmogorov Smirnov Test','T Test')
  sigma <- 1
  ...
  for(i in 1:length(d)){
    ...
    test.statistic <- replicate(R,{
      ...
      x <- rnorm(n1,d[i] + mu,sigma); y <- rnorm(n2,mu,sigma) #our sample
      ...
      c(MWU.statistic,WilcoxonRankSum.statistic,Kolmogorov_smirnov.statistic,T.statistic)
    })
    ...
    if(exact.crit[2]){
      ...
      WilcoxonRankSum.power <- mean((test.statistic[2,] - a - 0.5)/sqrt(b) 
                                    > cutpoint.2) #after continuity correction
    }
    
    #Simulated power for Kolmogorov Smirnov test
    if(exact.crit[3]){
      Kolmogorov_smirnov.power <- mean(test.statistic[3,] > exact.crit[3]) 
    }else{
    ...
    cutpoint.4 <- qt(alpha,n1+n2-2,lower.tail = F)
    T.power <- mean(test.statistic[4,] > cutpoint.4)
    
    #Storing the simulated powers in matrix
    power.matrix[i,] <-
      c(MNW.power,WilcoxonRankSum.power,Kolmogorov_smirnov.power,T.power)
  }
...
Visualize_Power_comparisonALL.1 <- function(n1,n2,d,mu,R,alpha,exact.crit 
                                            = c(F,F,F)){
  
  #To store power
  M <- Power_comparisonALL.1(n1,n2,d,mu,R,alpha,exact.crit)
  ...
  graph.1 <- ggplot(graph.data,aes(x,y,col = Index)) +
    ...
    labs(x = expression(paste('Difference ',(mu[1] - mu[2]))),y =
           'Simulated Power',subtitle = paste('Mu = ',mu,',n1 =',n1,',n2 = ',n2,',Alpha =',alpha,',R = ',R)) +
    ggtitle('Simulated Power Curve of Different Non Parametric Tests and T Test') +
    theme_bw(14) + scale_color_manual(values = wes_palette(n = 4,name = "Darjeeling1"))
